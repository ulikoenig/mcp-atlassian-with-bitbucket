...
```bash
# Restrict to core tools only (~76 tools across 11 core toolsets)
TOOLSETS=default
...
`bitbucket_repositories`, `bitbucket_pull_requests`, `bitbucket_branches`, `bitbucket_commits`, `bitbucket_source`
...
<Warning>
In v0.22.0, the default will change from all toolsets to 11 core toolsets only.
